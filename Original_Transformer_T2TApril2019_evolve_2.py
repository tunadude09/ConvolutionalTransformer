...
def dilated_causal_conv2d(inputs, filters, kernel_size=3, strides=1,
                     dilation_rate=1, padding="CAUSAL", num_heads=1, name=None,
                     hidden_size=None):
  if padding.lower() == "causal":
    #  TODO: I need dilated, causal 2d convolutions here
    #channel_size = inputs.get_shape().as_list()[-1]
    channel_size = hidden_size
  ...
  if name is None:
      ...
      name == str(str(name) + "dsdcc2d_")

  input_depth = hidden_size
  # input_depth = inputs.get_shape().as_list()[-1]
  initializer_stddev = input_depth ** -0.5
  kernel_shape = [kernel_size, kernel_size, input_depth, filters]
  w = tf.get_variable(
        str(str(name) + "_Wconv2d"), shape=kernel_shape,
        initializer=tf.random_normal_initializer(stddev=initializer_stddev))
...
def depthwise_sep_dilated_causal_conv1d(inputs, filters, kernel_size=3, strides=1,
                     dilation_rate=1, padding="CAUSAL", depthwise_sep=False, name=None):
...
#         name="conv")
     ...
     if name is None:
         ...
         name == str(str(name) + "dsdcc_")

     input_depth = inputs.get_shape().as_list()[-1]
     initializer_stddev = input_depth ** -0.5
     kernel_shape = [kernel_size, input_depth, filters]
     w = tf.get_variable(
          str(str(name) + "_Wcov1d"), shape=kernel_shape,
          initializer=tf.random_normal_initializer(stddev=initializer_stddev))
...
#
       ...
       #filter_mat = tf.Variable( tf.truncated_normal(shape=[kernel_size, channel_size, filters], stddev=0.1) )
       ##filter_mat = tf.zeros([kernel_size, channel_size, filters])
       #return tf.nn.conv1d(
    #        value=inputs,
#            filters=filter_mat,
#            stride=strides,
...
#
#
# #  og attention, sections following after are convtrans attention
#
# def compute_attention_component(antecedent,
#                                 total_depth,
#                                 filter_width=1,
...
#                 kv_padding="VALID",
#                 vars_3d_num_heads=0,
#                 layer_collection=None,v_kernel_size=None, v_dil_rate=None,
#                 v_padding=None, depthwise_sep=None):
#   """Computes query, key and value.
#   Args:
#     query_antecedent: a Tensor with shape [batch, length_q, channels]
#     memory_antecedent: a Tensor with shape [batch, length_m, channels]
...
def compute_attention_component(antecedent,
  ...
  def true_fn():
      return common_layers.dense(
                ...
                layer_collection=layer_collection)
  def false_fn():
      return depthwise_sep_dilated_causal_conv1d(
                inputs=antecedent,filters=total_depth, kernel_size=filter_width, padding=padding, name=name)

  def causal_decoder_fn():
      return depthwise_sep_dilated_causal_conv1d(
                inputs=antecedent,filters=total_depth, kernel_size=1, padding=padding, name=name)




  result1 = common_layers.dense(
                antecedent, total_depth, use_bias=False, name=name,
                layer_collection=layer_collection)
  result2 = depthwise_sep_dilated_causal_conv1d(
                inputs=antecedent,filters=total_depth, kernel_size=filter_width, padding=padding, name=name)

  result3 = depthwise_sep_dilated_causal_conv1d(
                inputs=antecedent,filters=total_depth, kernel_size=1, padding="SAME", name=name)





  # def not_causal_decoder_fn():
  #       return tf.cond(tf.equal(filter_width, 1), true_fn, false_fn)

  # return tf.cond(tf.equal(filter_width, 1), true_fn, causal_decoder_fn)

  #return not_causal_decoder_fn()
  #return tf.cond(tf.cast(causal_decoder_layer, tf.bool), causal_decoder_fn, not_causal_decoder_fn)
  # #return tf.cond(tf.cast(causal_decoder_layer, tf.bool), causal_decoder_fn, lambda: tf.cond(tf.equal(filter_width, 1), true_fn, false_fn))
  #
  # false_result = tf.cond(tf.cast(causal_decoder_layer, tf.bool), causal_decoder_fn, false_fn)
  #
  # def return_false_result():
  #     return false_result

  # return tf.cond(tf.equal(filter_width, 1), true_fn, return_false_result)

  #
  #
  # if causal_decoder_layer == True:
  #     return causal_decoder_fn()
  # else:
  #     return tf.cond(tf.equal(filter_width, 1), true_fn, true_fn)
  #
  ...
  #     return (true_fn)
  # else:
  #     return tf.cond(causal_decoder_layer == True, causal_decoder_fn, true_fn)




  # return tf.case([(tf.equal(filter_width, tf.constant(1)), true_fn),
  #                 (tf.equal(tf.cast(causal_decoder_layer, tf.bool), tf.constant(True)), causal_decoder_fn)],
...
#
  ...
  # return tf.case([(tf.equal(filter_width, tf.constant(1)), lambda: true_fn()),
  #                 (tf.equal(tf.cast(causal_decoder_layer, tf.bool), tf.constant(True)), lambda: causal_decoder_fn())],
  #                 default=lambda: false_fn(), exclusive=False)



  return tf.case([(tf.equal(filter_width, tf.constant(1)), lambda: result1),
                  (tf.equal(tf.cast(causal_decoder_layer, tf.bool), tf.constant(True)), lambda: result3)],
                  default=lambda: result2, exclusive=False)
  ...
  #
  #   #  FIXMETUNA: FIXMETUNA:
  #   #  FIXMETUNA:  PROBLEMS:  this area causes bugs:
  #   #  FIXME: conv not found in checkpoint
  #   return depthwise_sep_dilated_causal_conv1d(
  #       inputs=antecedent,filters=total_depth, kernel_size=filter_width, padding=padding, name=name
  #       )
        ###inputs, filters, kernel_size=3, strides=1,
        ###             dilation_rate=1, padding="CAUSAL", depthwise_sep=False
    # return common_layers.conv1d(
    #      antecedent, total_depth, filter_width, padding=padding, name=name)
...
def compute_qkv(query_antecedent,
  ...
  v = compute_attention_component(
      ...
      depthwise_sep=depthwise_sep,
      layer_collection=layer_collection,
      causal_decoder_layer=causal_decoder_layer)
  return q, k, v
...
def pointwise_attention(q,
  ...
  with tf.variable_scope(name, default_name="pointwise_attention", values=[q, k, v]) as scope:
    ...
    weights = dilated_causal_conv2d(weights, filters=v.get_shape().as_list()[-1], kernel_size=1,
                              strides=1, padding=padding, num_heads=num_heads, hidden_size=max_len)
...
#                                            gap_size, num_memory_blocks)
    ...
    if vars_3d:
      ...
      # FIXMETUNA: need to replace with the depthwise_sep_dilated_causal_conv1d if possible
      ...
      #
      #
      # kernel_shape = [v_kernel_size, x.get_shape()[-1], output_depth]
      # w = tf.get_variable(
      #   "DW", shape=kernel_shape)
      # x = tf.nn.conv1d(
      #   x,
...
class TransformerOriginalApril2019Evolve2(t2t_model.T2TModel):
  """Attention net.  See file docstring."""

  def __init__(self, *args, **kwargs):
    super(TransformerOriginalApril2019Evolve2, self).__init__(*args, **kwargs)
  ...
  def value_formation_module(self, input):
    ...
    input = depthwise_sep_dilated_causal_conv1d(input, self._hparams.value_formation_module_hidden_dim,
             kernel_size=3, strides=1, dilation_rate=1, padding="SAME", depthwise_sep=False, name=None)
    ...
    input = depthwise_sep_dilated_causal_conv1d(input, module_output_size,
         kernel_size=1, strides=1, dilation_rate=1, padding="SAME", depthwise_sep=False, name=None)
  ...
  def _greedy_infer(self, features, decode_length, use_tpu=False):
    ...
    if (self._target_modality_is_real or
      ...
      return super(TransformerOriginalApril2019Evolve2, self)._greedy_infer(features, decode_length)
